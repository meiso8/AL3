...
const char* sVersionString = "20240912";
...
void NoviceSystem::CreateGraphicsPipelines() {
	...
	pipelineSetTriangles_[kBlendModeExclusion] =
	    CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, kBlendModeExclusion);
	...
	pipelineSetLines_[kBlendModeExclusion] =
	    CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE, kBlendModeExclusion);
}
...
std::unique_ptr<NoviceSystem::PipelineSet> NoviceSystem::CreateGraphicsPipeline(
	...
	gpipeline.RTVFormats[0] = (blendMode != kBlendModeExclusion)
	                              ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB
	                              : DXGI_FORMAT_R8G8B8A8_UNORM; // 0～255指定のRGBA
...
void Novice::Initialize(const char* title, int width, int height, bool enableDebugLayer) {
	...
	sWinApp->CreateGameWindow(titleString, WS_OVERLAPPEDWINDOW & ~(WS_MAXIMIZEBOX | WS_THICKFRAME), width,
	    height);
	sWinApp->SetSizeChangeMode(WinApp::SizeChangeMode::kNone);

	// DirectX初期化処理
	sDxCommon = DirectXCommon::GetInstance();
	sDxCommon->Initialize(sWinApp, width, height, enableDebugLayer);
...
void Novice::DrawBox(
	...
	if (fillMode == kFillModeSolid) {
		...
		for (auto& vertex : vertices) {
			// 回転
			vertex = {
			    vertex.x * cosf(angle) + vertex.y * -sinf(angle),
			    vertex.x * sinf(angle) + vertex.y * cosf(angle)};
			// 平行移動
			vertex.x += static_cast<float>(x);
			vertex.y += static_cast<float>(y);
		}
		// 左上から右上
		sNoviceSystem->DrawLine(
		    int(vertices[1].x), int(vertices[1].y), int(vertices[2].x), int(vertices[2].y), color);
		// 左上から左下
		sNoviceSystem->DrawLine(
		    int(vertices[1].x), int(vertices[1].y), int(vertices[0].x), int(vertices[0].y), color);
		// 右上から右下
		sNoviceSystem->DrawLine(
		    int(vertices[2].x), int(vertices[2].y), int(vertices[3].x), int(vertices[3].y), color);
		// 左下から右下
		sNoviceSystem->DrawLine(
		    int(vertices[0].x), int(vertices[0].y), int(vertices[3].x), int(vertices[3].y), color);
	}
}
